...
test.describe('渲染与失败降级（spec §8）', () => {
  test('KaTeX 渲染：默认文档含 .katex；非法公式降级为源码 + 错误角标', async ({ page }) => {
    ...
    await expect(editor.locator('.katex-html').first()).toBeVisible()
    ...
    await expect(editor.locator('.typomd-math.typomd-node-error')).toBeVisible()
  })

  test('Mermaid 渲染为 SVG；非法源码降级为源码 + 错误角标', async ({ page }) => {
    await page.goto('/')
    const editor = page.locator('[data-testid="typomd"] .ProseMirror')
    await expect(editor.locator('.typomd-mermaid svg').first()).toBeVisible({ timeout: 15_000 })

    // 键盘键入 ``` 触发的是 code_block input rule 而非 mermaid 节点，同样经调试按钮注入
    await page.locator('[data-testid="doc-bad-mermaid"]').click()
    await expect(editor.locator('.typomd-mermaid.typomd-node-error')).toBeVisible({ timeout: 15_000 })
  })

  test('关闭 feature 后对应渲染消失（features 开关链路）', async ({ page }) => {
    await page.goto('/')
    await page.locator('[data-testid="feature-mermaid"]').click() // 重建编辑器
    await expect(page.locator('[data-testid="typomd"] .ProseMirror .typomd-mermaid')).toHaveCount(0)
  })
})
