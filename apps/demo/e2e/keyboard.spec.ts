...
test('工具栏键盘导航（§6）：真实 Tab 进工具栏、方向键移动、Enter 触发、Esc 返回', async ({ page }) => {
  ...
  const first = page.locator('.typomd-toolbar-button').first()
  await expect(first).toBeFocused()
  await expect(page.locator('.typomd-toolbar-button').nth(1)).toHaveAttribute('tabindex', '-1')
  ...
  await page.keyboard.press('Shift+Tab')
  await expect(page.locator('.typomd-toolbar-button[tabindex="0"]')).toBeFocused()
...
test('Slash 键盘全流程（§6）：打开/aria 指向/ Esc 关闭', async ({ page }) => {
  ...
  await expect(editor).toHaveAttribute('aria-activedescendant', 'typomd-slash-opt-heading')
  await page.keyboard.press('ArrowDown')
  await expect(editor).toHaveAttribute('aria-activedescendant', 'typomd-slash-opt-bold')
  await page.keyboard.press('Escape')
...
test('reduced-motion（§2.2）：浮层动画时长被压为 ~0', async ({ page }) => {
  ...
  const duration = await page.evaluate(() => {
    const el = document.querySelector('.typomd-slash')!
